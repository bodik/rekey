#!/bin/sh
# script will test self rekeying functionality on local keytab using a test principal hostx/self-fqdn
# usage: sh krb/tests/rekey_service_self.sh
...
${BASE}/rekey-self.py --keytab ${KEYTAB} --principal ${PRINCIPAL} --debug 2>&1 | grep -v "^INFO" | sh -ev
if [ $? -ne 0 ]; then
	rreturn 1 "$0 rekey"
fi
echo "========== INFO: rekey end"
...
echo "========== INFO: cleanup"
rm -f ${KEYTAB} ${KEYTAB}.new ${KRB5CCNAME}
unset KRB5CCNAME
